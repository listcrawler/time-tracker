...
- **Modals**: `src/time_tracker/modals.py` — `ConfirmModal` (quit-with-pending-changes dialog), `FilterModal` (filter entries by date range and text fields), `GroupingModal` (field selection), `BulkEditModal` (batch field update), `SearchModal` (full-text entry search), `CopyToBackendModal` (copy entries between backends)
